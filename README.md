...
- **FastaHandler** is a standalone Python application equipped with 26 sub-modules for interactive FASTA file manipulation, available as open-source (see [LICENSE](https://github.com/OZTaekOppa/FastaHandler/blob/main/README.md#license)).
...
This fork extends **FastaHandler** with a [Model Context Protocol (MCP)](https://www.anthropic.com/news/model-context-protocol) server, enabling all 26 modules to be executed through natural language prompts via LLM clients such as Claude Desktop and Cursor — no command-line expertise required.
...
```
"Generate assembly statistics for asm.fa"
→ Executes: all_fa_stats

"Rename FASTA headers using my mapping file rename_map.txt"
→ Executes: prefix_rename

"Filter sequences shorter than 100 bp, then calculate assembly statistics"
